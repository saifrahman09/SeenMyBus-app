...
if (document.getElementById('btn-submit-update')) {
    document.getElementById('btn-submit-update').onclick = async () => {
        ...
        try {
            ...
            // PASTE THIS TO REPLACE YOUR OLD NOTIFICATION BLOCK
if (Object.keys(notificationUpdates).length > 0) {
    try {
        await update(ref(db, 'broadcastNotifications'), notificationUpdates);
    } catch (notifErr) {
        // This stops the app from crashing for regular students!
        console.log("Automated push notification safely skipped (User is not Admin).");
    }
}
...
const valRouteBadge = document.getElementById('val-route-badge');
...
function showValidationCard(busInfo) {
    ...
    // --- UI REQ 2: Updated description text format ---
    if (valBusDetails) {
        const formattedSpot = busInfo.spotId.toUpperCase().replace('-', ' ');
        valBusDetails.textContent = `Bus number ${busDisplay} for ${formattedSpot} has been parked here.`;
    }
